"""
Raw event batch → FeatureOutput(cont=[64, N_CONT], cat=[N_CAT]).
...
@dataclass
class FeatureOutput:
    cont: torch.Tensor   # [MAX_SEQ_LEN, N_CONT] float32 — per-event continuous features
...
def featurize(payload: dict[str, Any], enriched: dict[str, Any] | None = None) -> FeatureOutput:
    ...
    cont_rows = [_event_to_cont(event, session) for event in events[:MAX_SEQ_LEN]]
    while len(cont_rows) < MAX_SEQ_LEN:
        cont_rows.append([0.0] * N_CONT)

    return FeatureOutput(
        cont=torch.tensor(cont_rows, dtype=torch.float32),
